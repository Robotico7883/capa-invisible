...
while (cap.isOpened()):
    ...
    mask_1=cv2.morphologyEx(mask_1,cv2.MORPHOLOGY_OPEN, np.ones((3,3),np.uint8))
    mask_1=cv2.morphologyEx(mask_1,cv2.MORPHOLOGY_DILATE, np.ones((3,3),np.uint8))  
    mask_2=cv2.bitwise_not(mask_1)
    res_1=cv2.bitwise_and(img,img,mask=mask_2)
    res_2=cv2.bitwise_and(img,img,mask=mask_1)
    final_output=cv2.addWeightd(res_1,1,res_2,-2,1,0)
